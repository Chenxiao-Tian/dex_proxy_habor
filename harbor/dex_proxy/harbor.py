...
class Harbor(DexCommon):
    ...
    def _register_endpoints(self, server: WebServer) -> None:
        ...
        for path in ("/public/markets", "/public/harbor/get_markets"):
            server.register("GET", path, self.get_markets, summary="List Harbor markets", tags=["public", "market-data"], oapi_in=["harbor"])

        # Account
        server.register("GET", "/public/account", self.get_account_raw, summary="Raw Harbor account payload", tags=["public", "account"], oapi_in=["harbor"])

        # Balances (never 404)
        for path in ("/public/balance", "/public/harbor/get_balance", "/api/harbor/balance", "/private/balance", "/public/exchange_balance"):
            server.register("GET", path, self.get_balance, summary="Get account balances", tags=["public", "balance"], oapi_in=["harbor"])

        # Legacy orders —— 不加 response_model，避免框架干预
        server.register("POST", "/private/create-order", self.create_order, summary="Create order (legacy)", tags=["private", "orders"], oapi_in=["harbor"])
        server.register("POST", "/private/harbor/create_order", self.create_order, summary="Create order (legacy)", tags=["private", "orders"], oapi_in=["harbor"])

        for method, path in (("DELETE", "/private/cancel-order"), ("DELETE", "/private/harbor/cancel_order"), ("POST", "/private/harbor/cancel_order")):
            server.register(method, path, self.cancel_order, summary="Cancel order (legacy)", tags=["private", "orders"], oapi_in=["harbor"])

        # ✅ 关键：公开 open orders 路由（上游 404 → 200 空数组）
        server.register("GET", "/public/orders", self.list_open_orders, summary="List open orders (legacy)", tags=["public", "orders"], oapi_in=["harbor"])
        server.register("GET", "/private/harbor/list_open_orders", self.list_open_orders, summary="List open orders (legacy)", tags=["private", "orders"], oapi_in=["harbor"])

        # Depth snapshot
        for path in ("/public/depth", "/public/harbor/get_depth_snapshot"):
            server.register("GET", path, self.get_depth_snapshot, summary="Depth snapshot for a symbol", tags=["public", "market-data"], oapi_in=["harbor"])
        ...
        try:
            ...
            if getattr(exc, "status_code", None) == 404:
                ack = HarborRequestAck(
                    request_id="",
                    status="NOOP",
                    client_request_id=query.request_type,
                    request_type=query.request_type,
                    order_id=None,
    ...
    async def get_all_open_requests(self, path, params, received_at_ms):
        ...
        try:
            ...
            if getattr(exc, "status_code", None) == 404:
                response = HarborOpenRequestsResponse(request_type=query.request_type, requests=[], send_timestamp_ns=str(now_ns()))
                return 200, response.to_dict()
            return self._error_response(query.request_type, None, exc)
        requests_payload = self._normalize_requests(api_response)
        response = HarborOpenRequestsResponse(request_type=query.request_type, requests=requests_payload, send_timestamp_ns=str(now_ns()))
        return 200, response.to_dict()
    ...
    async def get_markets(self, path, params, received_at_ms):
        try:
            ...
            return 200, {"count": len(self._markets), "markets": {"markets": list(self._markets.values())}, "send_timestamp_ns": str(now_ns())}
        except HarborAPIError as exc:
            status, err = self._harbor_error(exc)
            return status, {**err, "upstream_status": getattr(exc, "status_code", status)}
    ...
    async def get_balance(self, path, params, received_at_ms) -> Tuple[int, Dict[str, Any]]:
        ...
        try:
            ...
            meta = {"warning": "upstream_error", "error": {"code": exc.status_code, "message": exc.message}}
    ...
    async def create_order(self, path, params, received_at_ms) -> Tuple[int, Dict[str, Any]]:
        ...
        try:
            ...
            status, body = self._harbor_error(exc)
            return status, body
        except KeyError:
            return 400, {"error": {"message": f"Symbol {request.symbol} is not listed on Harbor"}}
        ...
        self._order_index[resp["client_order_id"]] = _OrderIndex(resp.get("symbol"), resp.get("order_id"))
        return 200, resp
    ...
    async def cancel_order(self, path, params, received_at_ms) -> Tuple[int, Dict[str, Any]]:
        ...
        self._order_index[p.client_order_id] = _OrderIndex(resp.get("symbol"), resp.get("order_id"))
        return 200, resp

    async def list_open_orders(self, path, params, received_at_ms) -> Tuple[int, Dict[str, Any]]:
        """
        GET /public/orders 以及 /private/harbor/list_open_orders
        - 上游 200：返回规范化订单数组
        - 上游 404：返回 200 + 空数组
        - 其它错误：结构化 JSON 错误
        """
        try:
            ...
            if getattr(exc, "status_code", None) == 404:
                return 200, {"send_timestamp_ns": str(now_ns()), "orders": []}
            status, body = self._harbor_error(exc)
            return status, body
        ...
        for o in orders:
            ...
            self._order_index[coid] = _OrderIndex(o.get("symbol"), o.get("order_id"))
        return 200, {"send_timestamp_ns": str(ts), "orders": orders}
    ...
    async def get_depth_snapshot(self, path, params, received_at_ms):
        ...
        try:
            ...
            if getattr(exc, "status_code", None) == 404:
                return 200, {"symbol": symbol, "lastUpdateId": "0", "bids": [], "asks": [], "send_timestamp_ns": str(now_ns())}
            status, err = self._harbor_error(exc)
            return status, {**err, "upstream_status": getattr(exc, "status_code", status)}

    # ------------------------------------------------------------------
    # DexCommon abstract hooks (unused in HTTP-only mode)
    # ------------------------------------------------------------------
    ...
    def on_request_status_update(self, client_order_id, request_status: RequestStatus, tx_receipt: dict, mined_tx_hash: str = None):
    ...
    def _error_response(self, request_type: Optional[str], client_request_id: Optional[str], exc: HarborAPIError) -> Tuple[int, Dict[str, Any]]:
        send_ts = str(now_ns())
        ...
        _LOGGER.warning("Harbor API error type=%s client_request_id=%s request_id=%s: %s", request_type, client_request_id, exc.request_id, message)
    ...
    def _map_order(self, order_payload: Dict[str, Any], *, send_timestamp: int) -> Dict[str, Any]:
    ...
    def _harbor_error(self, exc: HarborAPIError) -> Tuple[int, Dict[str, Any]]:
        ...
        return exc.status_code or 500, {"error": {"code": exc.status_code, "message": message, "request_id": exc.request_id}, "send_timestamp_ns": str(now_ns())}
