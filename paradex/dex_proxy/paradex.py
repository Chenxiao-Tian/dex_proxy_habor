...
from .paradex_jwt import ParadexJWT
...
class Paradex(DexCommon):
    def __init__(
        ...
        self._api = api_factory.create(self.pantheon, ConnectorType.Paradex)
        ...
        self.__jwt: Optional[ParadexJWT] = None
        ...
        try:
            async with aiohttp.ClientSession(json_serialize=ujson.dumps) as session:
                async with session.post(url, headers=headers) as response:
                    ...
                    if status_code == 200:
                        self.__jwt = ParadexJWT.from_string(response["jwt_token"])
                        return True
                    else:
                        self.__jwt = None
