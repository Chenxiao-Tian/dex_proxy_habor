...
from py_dex_common.dexes.whitelisting_manager_fordefi import WhitelistingManagerFordefi
from py_dex_common.dexes.whitelisting_manager_fireblocks import WhitelistingManagerFireblocks
...
class WhitelistingManagerTest:
    ...
    def _on_withdrawal_whitelist_refresh(self, withdrawal_address_whitelist: defaultdict):
        ...
        serializable_whitelist = dict(
            sorted(
                {k: sorted(list(v)) for k, v in withdrawal_address_whitelist.items()}.items()
            )
        )
        
        if "fordefi" in self.config:
            with open("fordefi.json", "w") as f:
                json.dump(serializable_whitelist, f, indent=4)
        else:
            with open("fireblocks.json", "w") as f:
                json.dump(serializable_whitelist, f, indent=4)
